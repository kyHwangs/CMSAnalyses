...
SSBGenInfor::SSBGenInfor(const edm::ParameterSet& iConfig)
    : isMC(iConfig.getParameter<bool>("isMCTag")), isSignal(iConfig.getParameter<bool>("isSignal")) {
...
void SSBGenInfor::GenMET(const edm::Event& iEvent, SSBTreeManager* ssbtreeManager) {
  ...
  for (GenMETCollection::const_iterator itgMET = genMETs->begin(); itgMET != genMETs->end(); itgMET++) {
    ...
    //        cout << "(*itgMET).genMET()->pt() ? " << (*itgMET).genMET()->pt() << endl;
    genMET_index++;
  }
