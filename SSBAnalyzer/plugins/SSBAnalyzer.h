...
/// needed for Egamma Scale Smear Syst.//
#include "DataFormats/EcalDetId/interface/ESDetId.h"
...
private:
  ...
  edm::EDGetTokenT<edm::ValueMap<bool>> pho_mva_NonTrigTightIdMapToken_;
  edm::EDGetTokenT<edm::ValueMap<float>> pho_mvaValuesMapToken_;
  ...
  bool isTight_calc;
  bool isHighPt_calc;
  bool isHighTrkPt_calc;

  bool tMuon_isPF; // = recoMu.isPFMuon()
  bool tMuon_isGlobal; // = recoMu.isGlobalMuon()
  bool tMuon_isTracker; // = recoMu.isTrackerMuon()

  double tMuon_ValidTrackHitFraction; // = recoMu.innerTrack()->validFraction()
  double tMuon_GlobalTrackChi2; // = recoMu.globalTrack()->normalizedChi2()
  double tMuon_TrackerSTAPositionMatch; // = recoMu.combinedQuality().chi2LocalPosition
  double tMuon_TrackKink; // = recoMu.combinedQuality().trkKink
  bool tMuon_TrackExist; // = recoMu.track().isNonnull()
  double tMuon_SegmentCompatibility; // = recoMu::segmentCompatibility(recoMu)

  double tMuon_GlobalMuonTrackChamberHit; // = recoMu.globalTrack()->hitPattern().numberOfValidMuonHits()
  double tMuon_TunePGlobalMuonTrackChamberHit; // = recoMu.tunePMuonBestTrack()->hitPattern().numberOfValidMuonHits()
  double tMuon_TunePpTError; // = recoMu.tunePMuonBestTrack()->ptError()
  double tMuon_TunePpT; // = recoMu.tunePMuonBestTrack()->pt()
  double tMuon_StationsHasSegments; // = recoMu.numberOfMatchedStations()

  double tMuon_dxy; // = fabs(recoMu.muonBestTrack()->dxy(vertex->position()))
  double tMuon_dB; // = dB() on pat::Muon
  double tMuon_dz; // = fabs(recoMu.muonBestTrack()->dz(vertex->position()))
  double tMuon_PixelHit; // = recoMu.innerTrack()->hitPattern().numberOfValidPixelHits()
  double tMuon_TrackLayerWithHit; // = recoMu.innerTrack()->hitPattern().trackerLayersWithMeasurement()
